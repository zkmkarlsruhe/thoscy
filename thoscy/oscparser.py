...
# parse an OSC message into a simple ThingsBoard key/value JSON payload for MQTT
# address as a str, arguments as a list
#
# single values: "/some/value 123" -> {"value": 123}
#      or
# multiple values:
...
#   /foo/bar 123       -> {"bar": 123}
#   /telemetry abc 123 -> {"abc", 123}
#
# returns json data on success or None on failure
def osc_to_json(address, args):
    components = osc_validate(address, args)
    if components == None: return None
    key = components[-1] # use trailing address component as key
    data = {}
...
# returns json data on success or None on failure
def osc_to_json_nested(address, args):
